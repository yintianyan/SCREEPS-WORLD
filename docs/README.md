...
| 改情报 / 侦察 / intel | [INTELLIGENCE_ARCHITECTURE.md](architecture/INTELLIGENCE_ARCHITECTURE.md)（**注意**：R11 后 A6 智能层 `intelligence-pipeline`/`decision-trace`/`evaluation-system` 为 Shadow-Only，不进入生产 bundle；生产观察采集在 `room-observer`/`prospect-manager`。恢复接线须走新 ADR，见该文件 §0） |
