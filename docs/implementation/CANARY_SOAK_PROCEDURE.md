...
| full deployment | small set 通过 | 持续运行 | 全量指标 | 见 §3 | 同上 + 人工灾难接管 | ✅ | — | 代码回滚 | 见 §6.1 |
...
| 维度 | 要求 | 当前状态 |
|------|------|---------|
| CPU 限制 | 模拟 MMO 20 CPU | [Blocked] 私服无限制 |
| tier 切换 | 验证四档降级 | [Blocked] |
| bucket 消耗 | 验证不枯竭 | [Blocked] |
| P3 饥饿旁路 | E2 触发 | [Blocked] |
...
| canary 硬性停止 | 确认回滚 | 是 |
| 人工灾难接管 | 停止自动发布 | 是 |
| 私服 reset 授权 | 明确授权后 reset | 是 |
| 线上 constructed wall 拆除授权 | 明确授权后拆除 | 是 |
| 线上核心建筑拆改授权 | 明确授权后拆改 | 是 |
| 参数校准 | tuning 覆盖层修改 | 是 |

### 6.2 禁止的人工介入

- ❌ 通过 flag/console 指挥 bot 运行
- ❌ 手动 spawn creep
- ❌ 手动创建 construction site（除授权拆除外）
- ❌ 手动修改 Memory（除 migration 修复外）
- ❌ 通过 LLM/外部 API 指挥 tick 决策
