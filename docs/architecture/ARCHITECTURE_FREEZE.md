...
| R21 | 2026-09-01 | 技术债修复（D1–D4：临时探针清理 + 纯函数下沉 + 横向 import 收敛） | **四项技术债修复**：①**D1** — `fill.ts` TEMP-3A-DIAG 临时探针移除（携能无填充目标是正常 FSM fallthrough，非异常态，`log` import 清除）；②**D4** — `kernel/layout-metrics.ts` 纯函数（`computeLayoutMetrics`/`computeDefenseMetrics` + 类型 `LayoutMetrics`/`DefenseCutInfo`）下沉 `src/domain/layout/metrics.ts`，kernel 侧仅保留 Memory 读写设施 + type re-export（KERNEL §9 登记表同步更新）；③**D2/D3** — layout-planner ↔ link-system 横向 import 收敛：纯逻辑（常量 + `transitionDismantlePlan`/`isDismantleOnCooldown`/`isLinkConstrained`/`isRoomInDefense`）下沉 `src/domain/layout/dismantle.ts`，link-system 仅保留 globalCache 操作封装作为 Public Interface，layout-planner/construction-manager 从 domain 获取纯逻辑、从 globalCache 直读 dismantlePlans（DEPENDENCY_GRAPH §3-7 登记更新、compliance 测试豁免注释同步）。`isRoomInDefense` 改为参数注入 `colonyState` 保持 domain 纯函数律。验证：typecheck 0 error / 4741 测试全绿 / build 成功。 | KERNEL_ARCHITECTURE §9 · DEPENDENCY_GRAPH §3-7 · ENGINEERING_BLUEPRINT §5-5 · SYSTEM_BOUNDARIES §2.3-2 · compliance.test.ts R1/R9 |
| R22 | 2026-09-01 | 技术债修复（D5–D6：过渡 barrel 清理 + e2e inject 全量迁移） | **两项技术债修复**：①**D5** — `role-helpers.ts` 过渡 barrel 残留清理：`role-helpers.ts` 在 T4 批①已升格为 `tests/support/factories.ts`，但文件已删除后仍有 3 处注释引用历史文件名。修复：`factories.ts` 注释更新为「已删除」；`fulfill-container-requests.test.ts`/`tier-a-regression.test.ts`/`TestWorld.ts` 注释中的 `role-helpers` 引用更新为 `factories`。grep 验证仓内零 `from.*role-helpers` import 残留。②**D6** — e2e 场景直调全量迁移到 `inject*` 具名 API：12 个 e2e 场景文件（04/12/15/17/18/20/21/22/23/24/25）中的全部直调（`runner.worldBuilder.addHostileCreep`/`addFriendlyCreep`/`addHostileTower`/`runner.addEnemyOwnedRoom`/`setUserGcl`/`setUserCpu`/`removeCreeps`）统一替换为 `inject*` 函数调用；新增 `injectHostileTower` 函数补全 API 覆盖。grep 验证仓内零直调残留。验证：typecheck 0 error / 4741 测试全绿 / build 成功。 | TEST_ARCHITECTURE §2/§4 · E2E_ENV_BASE_CONTRACT §2 · R19⑥ 触摸即迁移 |
| R23 | 2026-09-11 | 现状治理（权威源追认 + 防御战术登记） | **① powerBank 第二孵化权威源追认**：`power-farm-manager` 的 powerBank 野采编队（attacker/healer/pbCollector）向 spawnQueue 直接提交、不经 war-planner → 不落入 war 止损链。此路径上线已久但 agent.md «战争» 条款断言「attacker 仅由它孵化」从未修订，构成文档/代码权威分歧。裁决：追认该路径为既定第二孵化源（不回退），agent.md 战争条款同步标注例外；审计止损条款时须视 power-farm 为独立编队路径。**② remoteDefender 战术重构登记**：remoteDefender 从近战 ATTACK 改为 RANGED_ATTACK kiting 战术（边退边打、TOUGH 前置吸伤、RCL4~8 分档），对应 DEFENSE_ARCHITECTURE 防御者定位描述走强确认（无既有防御者行为冻结契约冲突，登记为战术演进）。**③ 塔优先孵化 defender 登记**：主房 defender 仅在无塔/小队威胁/塔能量不足时孵化（有塔有能量时单 NPC 由塔自动处理），收敛 SPAWN 防御者响应。**④ R9 compliance 白名单登记**：kernel → `creeps/support/targeting` 值导入 `getRoomThreatsCached`（共享 body-aware 威胁查询，消除 kernel 内嵌 find/isThreat 与角色层重复扫描），在 `compliance.test.ts` R9 白名单登记（与已有 `pruneDeadCreepCache` 例外同类权衡，详见 kernel.ts import 注释）。 | AGENT.md «战争» · DEFENSE_ARCHITECTURE · SPAWN_ARCHITECTURE · TEST_ARCHITECTURE §2 |
| R21 | 2026-09-01 | 技术债修复（D1–D4：临时探针清理 + 纯函数下沉 + 横向 import 收敛） | **四项技术债修复**：①**D1** — `fill.ts` TEMP-3A-DIAG 临时探针移除（携能无填充目标是正常 FSM fallthrough，非异常态，`log` import 清除）；②**D4** — `kernel/layout-metrics.ts` 纯函数（`computeLayoutMetrics`/`computeDefenseMetrics` + 类型 `LayoutMetrics`/`DefenseCutInfo`）下沉 `src/domain/layout/metrics.ts`，kernel 侧仅保留 Memory 读写设施 + type re-export（KERNEL §9 登记表同步更新）；③**D2/D3** — layout-planner ↔ link-system 横向 import 收敛：纯逻辑（常量 + `transitionDismantlePlan`/`isDismantleOnCooldown`/`isLinkConstrained`/`isRoomInDefense`）下沉 `src/domain/layout/dismantle.ts`，link-system 仅保留 globalCache 操作封装作为 Public Interface，layout-planner/construction-manager 从 domain 获取纯逻辑、从 globalCache 直读 dismantlePlans（DEPENDENCY_GRAPH §3-7 登记更新、compliance 测试豁免注释同步）。`isRoomInDefense` 改为参数注入 `colonyState` 保持 domain 纯函数律。验证：typecheck 0 error / 4741 测试全绿 / build 成功。 | KERNEL_ARCHITECTURE §9 · DEPENDENCY_GRAPH §3-7 · ENGINEERING_BLUEPRINT §5-5 · SYSTEM_BOUNDARIES §2.3-2 · compliance.test.ts R1/R9 |
