...
| R10 | 2026-08-27 | 现状治理（放宽+合并计划） | **System 上限从 15+3 调整为 36**（以 `bootstrap.ts` 实际 `registerSystem` 调用数为唯一真相源），合并前 43 个 System 分三批合并：(1) A6 智能层 6→1 `intelligence-pipeline`（Shadow-Only post 系统，无 Game API、无执行行为，按内部分频串行 6 阶段）✅ 已完成；(2) A5.4 战术运行时 4→1 `tactical-runtime-pipeline`（P2 main，仅在 war 姿态下运行，按内部分频串行 4 阶段）✅ 已完成；(3) 非蓝图 System 3→0 合并（specialization-planner→empire-strategy，logistics-planner→logistics，empire-health→self-healing）⏳ 后置。批 1+2 完成后 `bootstrap.ts` 实际 36 个 `registerSystem` 调用（理论 35，实际 36，差异来自历史计数口径）。剩余超量因各模块有独立状态所有权/CPU 节奏/降级序不可再合并。批 3 可后置。 | SYSTEM_BOUNDARIES §2 |
| R11 | 2026-08-28 | 现状治理（删减式重构登记） | **intelligence-pipeline / decision-trace / evaluation-system 从生产路径移除**：删减式重构移除了 `src/kernel/decision-trace.ts`、`src/systems/evaluation-system.ts`、`src/telemetry/EvaluationRegistry.ts` 及对应 barrel export。`bootstrap.ts` 不再注册这三个系统，实际注册数从 36 降为 34。globalCache 中相关字段降级为 `unknown` 或保留为观测槽位。**现状判定**：intelligence/evaluation 模块源码仍存在于 `src/domain/intelligence/` 但不进入生产 bundle，测试仍验证源码模块而非生产路径。**待裁决**：(A) 恢复注册并接线生产路径；或 (B) 正式从冻结目标删除并清理源码和测试。在裁决前保持 Shadow-Only 不接线路径。 | SYSTEM_BOUNDARIES §2 · ENGINEERING_BLUEPRINT §5 |
