...
| W1 | war 授权硬门槛接入：war-planner/war-planning-system 目标选择切换 `intelActionUsable`（fact 级 + 年龄上限）；demobilize 战后核验非 fact 降级 unknown | FREEZE R18 | 行为保持四件套；stale-but-fresh intel 不再授权（INTELLIGENCE §5） | ⏳ |
| W2 | 诱饵对抗场景（Scenario F）：空城伪装/诱饵塔不触发授权 | FREEZE R18 | 诱饵不触发授权（Phase 9 验收） | ⏳ |
| W3 | war 账本证据：战争全程经济不越红线的 e2e 战例 | FREEZE R18 | 战争全程经济不越红线（Phase 9 验收） | ⏳ |
| W4 | 止损链实测：spawned 超限收摊 / warBlacklist 冷却 / 满编才 advance | FREEZE R18 | 止损触发即收摊；满编才推进（Phase 9 验收） | ⏳ |
