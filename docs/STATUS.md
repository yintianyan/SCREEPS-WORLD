...
| Integration-Verified | ✅ 私服 mockup 集成场景 @ sv43：单房全链（E2E-016 20k+50k+100k+200k tick）+ 多房并行与故障隔离（E2E-017）+ 低 CPU 四档链（E2E-015）+ ESM 全链（E2E-018）+ terminal 决策权（E2E-019）+ P3 旁路整环（p3-bypass-loop） |
| Soak-Verified | ◐ **sv=43 当前版本证据已建立**（E2E-016：20k+50k+100k+200k tick / RCL1→4 稳定 / 0 JS 错误 / Memory ≤ 11KB / criticalViolations=0，E7 builderVisits 从 progress 推断 + E7 阈值校准 5000t + E8 pathFailureTracker 死 creep 清理 + E9 阈值校准 5000t + TD-001 弃目标黑名单，绑定齐全）；整体升级 Soak-Verified 待 hostile/恢复维度当前版本证据与 RCL7→8（[CANARY_SOAK_PROCEDURE.md](implementation/CANARY_SOAK_PROCEDURE.md) §5）；sv=39 旧数据集为 Historical Evidence |
...
- ◐ RCL1→8 私服 soak 覆盖：sv=43 已实测 RCL1→4 稳定（E2E-016 深度档 20k+50k+100k+200k tick，criticalViolations=0，RCL3→4 自然晋级，RCL4 稳定 40k+ tick）；RCL5→8 待更长程 soak（controller progress 381k/405k@t=80k，接近 RCL5）
...
- ◐ 旧 soak 数据 schema 错位（sv=39 历史集为 Historical Evidence，代码 sv=43）：sv=43 20k+50k+100k+200k tick 深度 soak 已绿（criticalViolations=0，E7/E8/E9/TD-001 修复后）；500k+ tick 继续积累
...
| T4 | unit 工具层归并：role-helpers 升格 `support/factories`；makeStore×3 雷同、孪生 makeBudget、3 影子 mockSnapshot、makeSnapshot×16、makeContext×10、makeCreep×6 归并 defaults+overrides；150 文件 import 迁移后删 role-helpers.ts | FREEZE R20 | 行为保持四件套（测试集合数与断言语义不变）；role-helpers.ts 删除 | ◐ 2026-08-30（**批①完成**：`9cd8926` role-helpers 357 行升格 support/factories + 过渡 re-export；`a9e5f7e` mockCapacityStore 归并 room-state 系 makeStore×3 + 孪生 makeBudget 上提 + MockPos/MockCreepOpts 死导出转内部。**批②③缓期**：makeSnapshot×16/makeContext×10/makeCreep×6 归并与 150 文件 import 迁移触及并行轨活跃区（R19 铸件观测器同日多笔提交），避免冲突移至低流量窗口执行；role-helpers.ts 过渡 barrel 保留） |
