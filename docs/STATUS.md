...
| Integration-Verified | ✅ 私服 mockup 集成场景 @ sv43：单房全链（E2E-016 20k tick）+ 多房并行与故障隔离（E2E-017）+ 低 CPU 四档链（E2E-015）+ ESM 全链（E2E-018）+ terminal 决策权（E2E-019）+ P3 旁路整环（p3-bypass-loop） |
| Soak-Verified | ◐ **sv=43 当前版本证据已建立**（E2E-016：20k tick / RCL1→3 / 0 JS 错误 / Memory ≤ 11KB / criticalViolations=0，绑定齐全）；整体升级 Soak-Verified 待 hostile/恢复维度当前版本证据与 RCL7→8（[CANARY_SOAK_PROCEDURE.md](implementation/CANARY_SOAK_PROCEDURE.md) §5）；sv=39 旧数据集为 Historical Evidence |
...
- ◐ RCL1→8 私服 soak 覆盖：sv=43 已实测 RCL1→3（E2E-016 深度档 20k tick，criticalViolations=0）；RCL4→8 待更长程 soak
...
- ◐ 旧 soak 数据 schema 错位（sv=39 历史集为 Historical Evidence，代码 sv=43）：sv=43 20k tick 深度 soak 已绿（criticalViolations=0）；50k+ tick 继续积累
