...
- ✅ 市场交易链路验证（E2E-026 @ sv43，2026-08-31）：NPC 订单 DB 注入 → Game.market.getAllOrders 返回 1 sell order → deal 成交结算全链验证通过（bot credits 100000→99920 扣减、NPC terminal energy 110000→108000 出货、order remainingAmount 100000→98000 被吃 2000=2 次 deal × maxDealAmount=1000、terminal cooldown=302 设置）；动态定价系统（market-pricing + deal-scheduler + terminal-policy）纯函数层全链路审查通过；降级路径：tier≠healthy/guardied 或 bucket<minBucket 时 terminal-manager 跳过；global reset 恢复：marketPrices heap 存储，reset 后首 tick refreshMarketPrices 重建，fallback 值兜底
...
| T4 | unit 工具层归并：role-helpers 升格 `support/factories`；makeStore×3 雷同、孪生 makeBudget、3 影子 mockSnapshot、makeSnapshot×16、makeContext×10、makeCreep×6 归并 defaults+overrides；150 文件 import 迁移后删 role-helpers.ts | FREEZE R20 | 行为保持四件套（测试集合数与断言语义不变）；role-helpers.ts 删除 | ✅ 2026-09-01（**批①完成**：`9cd8926` role-helpers 357 行升格 support/factories + 过渡 re-export；`a9e5f7e` mockCapacityStore 归并 room-state 系 makeStore×3 + 孪生 makeBudget 上提 + MockPos/MockCreepOpts 死导出转内部。**批②完成**：`dd6d980` 新增 `mockRoomStateCtx` 共享工厂，4 个 room-state 单元测试 makeCtx 归并（room-state/src-ratio/defense-stale/observer），清理 TickContext 死 import，4727 测试全绿。**批②审计结论**：其余 31 个本地 makeSnapshot/makeContext/makeCreep 工厂签名和返回类型各异（TacticalSnapshot/EmpireSnapshot/RecoveryWorldSnapshot 等不同类型），属于领域特定构造器，不可安全归并。**批③完成**：`role-helpers.ts` 已删除，全部 import 已迁移到 `support/factories`；3 个注释中的历史提及更新为 `factories.ts`） |
| T5 | 孤儿治理：integration Assertions 10 个零调用方法、tickRunner 单例、stuckCreeps、两 barrel 22 个死 re-export、SnapshotInspector 零调用方法、disasterRoom、run-e2e.mjs/smoke-e2e.cjs 删除；e2e/setup.ts 与 rebuild-driver-snapshot.js 双实现合一；fixtures/inject.ts 依 R19⑥ 触摸即迁移启用（16/25/26 soak 直调点先行） | FREEZE R20 · R19⑥ | 行为保持四件套；零引用导出清单清零或转在用 | ✅ 2026-09-01（**已完成**：Assertions 10 方法+工厂、GameInspector.stuckCreeps、tickRunner 单例、SnapshotInspector 6 查询方法、两 barrel 22 死 re-export、run-e2e.mjs/smoke-e2e.cjs 删除（`a9e5f7e`+`80bd7d1`）；inject.ts 转在用——injectSpawnEnergy/injectFriendlyTower 落地 07/04。**缓期项完成**：disasterRoom 删除（01-disaster-recovery 改用 standardRoom 直调）、e2e/setup.ts↔rebuild-driver-snapshot.js 双实现合一（setup 委托 scripts 调用，消除 60 行重复逻辑）（`eef16fa`）。**存量场景 inject 全量迁移完成**：新增 `injectHostileTower` 函数；12 个 e2e 场景文件中的全部直调（`runner.worldBuilder.addHostileCreep`/`addFriendlyCreep`/`addHostileTower`/`runner.addEnemyOwnedRoom`/`setUserGcl`/`setUserCpu`/`removeCreeps`）统一迁移到 `inject*` 具名注入 API；grep 验证仓内零直调残留） |
...
| TD-002 ✅ | TEMP-3A-DIAG 临时探针残留 | `fill.ts` fillTarget() 中的 `log.warn` 诊断探针（携能无填充目标时触发） | 每 tick 产生噪音日志，无行为影响 | ✅ 2026-09-01 已修复：探针移除，`log` import 清除。携能无填充目标是正常 FSM fallthrough 条件（resolve→undefined→下一候选），不需要诊断 |
| TD-003 ✅ | kernel/layout-metrics.ts 承载布局业务语义 | 纯函数 `computeLayoutMetrics`/`computeDefenseMetrics` + 类型 `LayoutMetrics`/`DefenseCutInfo` 位于 kernel 层，违反 KERNEL §9「不承载业务语义」 | kernel 层含布局业务纯函数，架构层级混淆 | ✅ 2026-09-01 已修复：纯函数+类型下沉 `src/domain/layout/metrics.ts`，kernel 侧仅保留 Memory 读写设施（recordLayoutMetrics/readDefenseCutPositions）+ type re-export |
| TD-004 ✅ | layout-planner ↔ link-system 横向 import 违规 | layout-planner 和 construction-manager 直接 import link-system 的 dismantle 函数（`isDismantleOnCooldown`/`isLinkConstrained`/`isRoomInDefense`/`transitionDismantlePlan`/`getDismantlePlans`/`clearDismantlePlan`），违反 DEPENDENCY_GRAPH §3-7「禁止兄弟系统横向 import 直读内部状态」 | 系统间耦合违规，架构边界模糊 | ✅ 2026-09-01 已修复：纯逻辑（常量 + `transitionDismantlePlan`/`isDismantleOnCooldown`/`isLinkConstrained`/`isRoomInDefense`）下沉 `src/domain/layout/dismantle.ts`；link-system 仅保留 globalCache 操作封装作为 Public Interface；layout-planner/construction-manager 从 domain 获取纯逻辑，globalCache 直读 dismantlePlans；DEPENDENCY_GRAPH §3-7 登记更新 |
| TD-005 ✅ | role-helpers.ts 过渡 barrel 残留 | T4 批③缓期项：`role-helpers.ts` 已删除但 3 个文件注释中仍有历史提及 | 注释引用已删除文件，误导维护 | ✅ 2026-09-01 已修复：`factories.ts` 注释更新为「已删除」；`fulfill-container-requests.test.ts`/`tier-a-regression.test.ts`/`TestWorld.ts` 注释中的 `role-helpers` 引用更新为 `factories` |
| TD-006 ✅ | e2e 场景直调未走 inject API | 12 个 e2e 场景文件直接调用 `runner.worldBuilder.addHostileCreep`/`addFriendlyCreep`/`addHostileTower`/`runner.addEnemyOwnedRoom`/`setUserGcl`/`setUserCpu`/`removeCreeps`，未使用 `fixtures/inject.ts` 具名注入 API | 注入操作分散在场景中直调底层 API，缺少统一具名登记和语义分层 | ✅ 2026-09-01 已修复：新增 `injectHostileTower` 函数；12 个场景文件全部迁移到 `inject*` 具名 API（04/12/15/17/18/20/21/22/23/24/25）；grep 验证仓内零直调残留 |
