...
| Soak-Verified | ◐ **sv=43 当前版本证据已建立**（E2E-016：20k+50k+100k+200k+300k tick / RCL1→5 自然晋级 / 0 JS 错误 / Memory ≤ 10KB / criticalViolations=0，E7 builderVisits + E7 阈值校准 5000t + E8 死 creep 清理 + E9 阈值校准 5000t + TD-001 弃目标黑名单 + repathLimit 2→4 调优，绑定齐全）；整体升级 Soak-Verified 待 hostile/恢复维度当前版本证据与 RCL7→8（[CANARY_SOAK_PROCEDURE.md](implementation/CANARY_SOAK_PROCEDURE.md) §5）；sv=39 旧数据集为 Historical Evidence |
