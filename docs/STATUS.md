...
| T1 | tests/support 统一脚手架内核 + 常量 SSOT 接线：`@screeps/driver` constants 升显式 devDep；setup.ts 私有 FIND 编码退役改注入官方值；TestWorld 本地常量表（RCL_PROGRESS/PART_COST/BUILD_COST/FIND_*）删除改 import；e2e parity 哨兵场景落地 | FREEZE R20 | 行为保持四件套；被假常量污染的 integration 断言预期值修正逐例登记（R20④豁免条款）；parity 场景绿 | ⏳ |
| T2 | e2e 冒烟进 deploy 门禁（§4 PR 编排落地）+ AGENT.md 命令清单对齐 §2 口径 + bundle parity 守卫归一 integration 侧（§2 登记位置修订） | FREEZE R20 | CI deploy workflow 含 smoke 步骤且绿；compliance.test.ts dist-skip 分支删除 | ⏳ |
| T3 | 断言/错误过滤/census 归并：e2e `isJsError` 26 处 → `support/errors` 唯一实现；`SnapshotInspector.structureCensus()` 新增；16-soak 内联 CENSUS 字符串改 inspector 查询（证据行输出格式不变） | FREEZE R20 | 行为保持四件套；grep 验证仓内 `function isJsError` 本地定义为 0 | ⏳ |
| T4 | unit 工具层归并：role-helpers 升格 `support/factories`；makeStore×3 雷同、孪生 makeBudget、3 影子 mockSnapshot、makeSnapshot×16、makeContext×10、makeCreep×6 归并 defaults+overrides；150 文件 import 迁移后删 role-helpers.ts | FREEZE R20 | 行为保持四件套（测试集合数与断言语义不变）；role-helpers.ts 删除 | ⏳ |
| T5 | 孤儿治理：integration Assertions 10 个零调用方法、tickRunner 单例、stuckCreeps、两 barrel 22 个死 re-export、SnapshotInspector 零调用方法、disasterRoom、run-e2e.mjs/smoke-e2e.cjs 删除；e2e/setup.ts 与 rebuild-driver-snapshot.js 双实现合一；fixtures/inject.ts 依 R19⑥ 触摸即迁移启用（16/25/26 soak 直调点先行） | FREEZE R20 · R19⑥ | 行为保持四件套；零引用导出清单清零或转在用 | ⏳ |
| T6 | 标题-断言对齐：07-energy-crisis 前提真实化（新增 `injectSpawnEnergy` 具名注入，R19② 白名单登记）；03-storage-construction 补 storage site 断言；04-tower-defense 补 hostile hits 下降断言（world.roomObjects 真值） | FREEZE R20 · R19② | 三场景断言与标题一致且绿 | ⏳ |
