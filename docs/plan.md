...
#### 塔防侦察兵修复与无害侦察观测（R7c，schema v32，已实施）

- **bug 修复**：无害敌对（侦察兵，无威胁部件）在场时，塔防曾走维修分支 —
  显式 `tower.repair` 占用塔的本 tick 动作、抑制引擎自动攻击，导致满能量塔
  对贴身侦察兵不开火。修复：无害敌对在场时塔不接任何任务（放空让引擎
  自动点杀），威胁开火路径与维修链均不受影响。
- **盯防观测**：room-state 在「有敌对但无威胁部件」时写入
  `lastObserverAt` + `observerSightings`（有界累计），与 lastHostileAt 威胁
  记忆刻意分离 — 不触发防御/姿态，纯情报；首次目击与每 500 tick 限流日志。
  持续目击 = 有人盯防的信号（可为 war 情报预热）。
