...
文档导航见 [docs/README.md](docs/README.md)，技术债台账见
[docs/implementation/TECH_DEBT_LEDGER.md](docs/implementation/TECH_DEBT_LEDGER.md)。
...
文档分三层（详见 [docs/README.md](docs/README.md)）：`docs/architecture/` 冻结蓝图、
`docs/research/` 调研存档、`docs/implementation/` 实施记录。
...
- **已登记例外（R9）**：kernel 直接 import 业务侧 `pruneDeadCreepCache`（100 tick
  低频钩子）；出现 3+ 个维护钩子时必须提取 registry 钩子机制。
