// 补丁说明：本文件通过 mods.json 替换 node_modules/screepsmod-auth/index.js 加载
// require 路径指向 node_modules/screepsmod-auth/lib/ 下的原始文件
const _lib = '/screeps/node_modules/screepsmod-auth/lib/'
const Auth = require(_lib + 'auth')
const crypto = require('crypto')
const { httpEndpointKey } = require(_lib + 'http-endpoint')
const { getTokenRateLimitState, getActiveTokenRateLimitState } = require(_lib + 'ratelimit')
...
module.exports = function (config) {
  ...
  if (config.mongo && config.mongo.client) {
    // MongoDB mode
  ...
  require(_lib + 'config')(config)
  if (config.backend) require(_lib + 'backend')(config, auth)
}
