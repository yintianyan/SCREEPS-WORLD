...
describe("水位表 — demand 编制信号（D-3/B-5）", () => {
  ...
  it("D-3：storage < full(50k) 时 tower 不计入 distributor 需求信号", () => {
    ...
    const withTowers = evaluateDemand(
      snapWithTowers,
      [],
      "normal",
      livingHarvester(),
    ...
    const spawnOnly = evaluateDemand(
      snapSpawnOnly,
      [],
      "normal",
      livingHarvester(),
...
describe("水位表 — getSource 平局去偏置（B-4）", () => {
  ...
  it("跨 tick 稳定：拥挤迁移后下一 tick 不翻转", () => {
    ...
    const occ2 = new Map<string, number>([
      ["s1", 0],
      ["s2", 0],
    ]);
    if (c1.memory.sourceId)
      occ2.set(c1.memory.sourceId as string, (occ2.get(c1.memory.sourceId as string) ?? 0) + 1);
    if (c2.memory.sourceId)
      occ2.set(c2.memory.sourceId as string, (occ2.get(c2.memory.sourceId as string) ?? 0) + 1);
