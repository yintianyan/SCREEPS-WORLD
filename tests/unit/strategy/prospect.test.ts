...
} from "../../../src/domain/strategy/prospect";
...
describe("selectProspectTarget", () => {
  ...
  it("视野已新鲜（sources 已知且未过期）→ 跳过，不浪费侦察", () => {
    const t = selectProspectTarget([candidate({ sources: 2, lastSeen: TICK - 10 })], TICK, OPTS);
    expect(t).toBeUndefined();
  });

  it("sources 已知但已过期 → 仍需侦察", () => {
    const t = selectProspectTarget([candidate({ sources: 2, lastSeen: TICK - 200 })], TICK, OPTS);
  ...
  it("过滤：SK/中心房、有主房、他人预定房、被占用房", () => {
    expect(selectProspectTarget([candidate({ kind: "sk" })], TICK, OPTS)).toBeUndefined();
    expect(selectProspectTarget([candidate({ owner: "Enemy" })], TICK, OPTS)).toBeUndefined();
    expect(selectProspectTarget([candidate({ reservedBy: "Other" })], TICK, OPTS)).toBeUndefined();
    expect(selectProspectTarget([candidate({ occupied: true })], TICK, OPTS)).toBeUndefined();
  });

  it("己方预定（reservedBy = myUsername）不排除", () => {
    const t = selectProspectTarget([candidate({ reservedBy: "Me" })], TICK, OPTS);
    expect(t?.roomName).toBe("W6N4");
  });
  ...
  describe("视野外扩（known=false 前沿发现）", () => {
    ...
    it("已知房有主（被跳过）时，前沿发现候选兜底选中", () => {
      const t = selectProspectTarget([candidate({ owner: "Enemy" }), frontier("W7N3")], TICK, OPTS);
    ...
    it("多个前沿发现候选按距离选最近", () => {
      const t = selectProspectTarget([frontier("W9N4"), frontier("W7N3")], TICK, OPTS);
      expect(t?.roomName).toBe("W7N3");
    });

    it("已知房与前沿候选同距时，已知房优先（保持重探语义）", () => {
      const t = selectProspectTarget([candidate(), frontier("W7N3")], TICK, OPTS);
      expect(t?.roomName).toBe("W6N4");
    });
