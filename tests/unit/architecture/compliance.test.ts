...
describe("R1 domain 纯度", () => {
  ...
  it("代码行不得引用 Game./Memory./console.", () => {
    ...
    for (const f of domainFiles) {
      ...
      if (/Game\./.test(code) || /Memory\./.test(code) || /console\./.test(code))
  ...
  it("禁 roles/engine", () => {
    ...
    for (const f of systemFiles) {
      for (const imp of importsOf(f)) {
        ...
        if (!rel.includes("creeps/movement/") && !rel.includes("creeps/support"))
          bad.push(`${relative(SRC, f)} -> ${rel}`);
      }
    }
    expect(bad, bad.join(NL)).toHaveLength(0);
  });
...
describe("R6 roles 动作红线", () => {
  ...
  it("禁 spawnCreep/createConstructionSite/PathFinder.search/Game.market", () => {
    ...
    for (const f of roleFiles) {
      ...
      if (/createConstructionSite\s*\(/.test(code))
        bad.push(`${relative(SRC, f)}: createConstructionSite`);
...
describe("R8 roles 禁止 Room.find 和全局扫描", () => {
  ...
  it("禁 room.find / Object.values(Game.creeps) / Game.getObjectById", () => {
    ...
    for (const f of roleFiles) {
      ...
      const code = codeLines(readFileSync(f, "utf8"));
      if (
        /(?:^|[^.])\broom\.find\s*\(|creep\.room\.find\s*\(/.test(code) &&
        !R8_EXEMPTIONS.has(`${rel}:room.find`)
      ) {
        bad.push(`${rel}: room.find`);
      }
      if (
        /Object\.values\(Game\.creeps\)/.test(code) &&
        !R8_EXEMPTIONS.has(`${rel}:Object.values(Game.creeps)`)
      ) {
  ...
  it("值导入业务模块仅在白名单中", () => {
    ...
    for (const f of kernelFiles) {
      ...
      for (const imp of importsOf(f)) {
        ...
        if (targetLayer !== "systems" && targetLayer !== "creeps" && targetLayer !== "domain")
          continue;
...
describe("R10 bootstrap 注册集合一致性", () => {
  ...
  const systemCalls = [...bootstrapSrc.matchAll(/\.registerSystem\(\s*(\w+)\s*\)/g)].map(
    m => m[1]!,
  );
...
describe("R11 systems 间值导入审计", () => {
  ...
  const R11_WHITELIST = new Set<string>([
    ...
    "systems/room/construction-manager.ts:systems/site-quota.ts",
    "systems/remote/remote-mining-manager.ts:systems/site-quota.ts",
    // 公开查询函数
    "systems/empire/agenda-manager.ts:systems/room/economy.ts",
    "systems/empire/empire-economy.ts:systems/room/economy.ts",
    "systems/empire/expansion-planner.ts:systems/empire/empire-economy.ts",
    // IntelQuery 公开查询函数（消费者只读 API）
    "systems/military/war-planner.ts:systems/intelligence.ts",
    "systems/military/war-planning-system.ts:systems/intelligence.ts",
    "systems/military/tactical-runtime-system.ts:systems/intelligence.ts",
    "systems/military/power-farm-manager.ts:systems/intelligence.ts",
    "systems/empire/prospect-manager.ts:systems/intelligence.ts",
    "systems/empire/empire-strategy.ts:systems/intelligence.ts",
    "systems/empire/specialization-planner.ts:systems/intelligence.ts",
    "systems/empire/expansion-manager.ts:systems/intelligence.ts",
    "systems/empire/expansion-planner.ts:systems/intelligence.ts",
    "systems/remote/remote-mining-manager.ts:systems/intelligence.ts",
    "systems/room-observer.ts:systems/intelligence.ts",
    // link-system 暴露的拆改/死资产协调接口
    "systems/room/layout-planner.ts:systems/room/link-system.ts",
    "systems/room/construction-manager.ts:systems/room/link-system.ts",
    // tactical-runtime-pipeline 编排其 4 个子阶段
    "systems/military/tactical-runtime-pipeline.ts:systems/military/tactical-runtime-system.ts",
    "systems/military/tactical-runtime-pipeline.ts:systems/military/squad-movement-runtime.ts",
    "systems/military/tactical-runtime-pipeline.ts:systems/military/tactical-engagement-runtime.ts",
    "systems/military/tactical-runtime-pipeline.ts:systems/military/combat-micro-runtime.ts",
    ...
    "systems/military/combat-micro-runtime.ts:systems/military/squad-movement-runtime.ts",
    // terminal-manager 拆分出的子模块：manager 编排 market/selfaid，
    // selfaid 复用 market 的库存查询（collectMineralInventory）
    "systems/trade/terminal-manager.ts:systems/trade/terminal-market.ts",
    "systems/trade/terminal-manager.ts:systems/trade/terminal-selfaid.ts",
    "systems/trade/terminal-selfaid.ts:systems/trade/terminal-market.ts",
    // expansion-manager 拆分出的子模块：manager 编排 Plan 消费/状态机/UOEM/自举车道
    "systems/empire/expansion-manager.ts:systems/empire/expansion/uoem-events.ts",
    "systems/empire/expansion-manager.ts:systems/empire/expansion/bootstrap-lane.ts",
    "systems/empire/expansion-manager.ts:systems/empire/expansion/plan-adapter.ts",
    "systems/empire/expansion-manager.ts:systems/empire/expansion/state-machine.ts",
    // state-machine 编排 plan-adapter（预算查询）与 uoem-events（终态出口），
    // plan-adapter 消费 IntelQuery（公开查询函数）
    "systems/empire/expansion/state-machine.ts:systems/empire/expansion/plan-adapter.ts",
    "systems/empire/expansion/state-machine.ts:systems/empire/expansion/uoem-events.ts",
    "systems/empire/expansion/plan-adapter.ts:systems/intelligence.ts",
    "systems/empire/expansion/plan-adapter.ts:systems/empire/expansion/uoem-events.ts",
    // remote-mining-manager 拆分出的子模块：manager 编排四块，
    // road-planner 复用账本结构成本（op-lifecycle）与 dismantle 回收（creep-recycle）
    // 及共享账本 site-quota
    "systems/remote/remote-mining-manager.ts:systems/remote/op-lifecycle.ts",
    "systems/remote/remote-mining-manager.ts:systems/remote/creep-recycle.ts",
...
// R12 production bundle parity 已收敛至 tests/integration/framework/bundle-parity.test.ts
//（FREEZE R20③：dist 仅在 build 后存在，守卫必须在 build 之后运行——unit 层在
// CI 中先于 build，恒走 skip 分支形同虚设；「已删模块不回流」检查一并迁移）。
