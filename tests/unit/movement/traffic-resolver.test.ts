...
describe("resolveTraffic — 同格仲裁", () => {
  it("多意图争同一格：最高优先级胜，败者原地", () => {
    const { moves } = resolveTraffic(
      input({
        intents: [intent("low", 100, 200, 40), intent("high", 300, 200, 60)],
        occupancy: new Map([
          [100, "low"],
          [300, "high"],
        ]),
  ...
  it("同优先级平局：登记序在前者胜", () => {
    const { moves } = resolveTraffic(
      input({
        intents: [intent("first", 100, 200, 40), intent("second", 300, 200, 40)],
        occupancy: new Map([
          [100, "first"],
          [300, "second"],
        ]),
...
describe("resolveTraffic — 跟车与对向换位", () => {
  it("跟车链：A 等 B 的格、B 走空格 — 多轮传播后全部放行", () => {
    ...
    const { moves } = resolveTraffic(
      input({
        intents: [intent("c", 100, 200, 40), intent("b", 200, 300, 40), intent("a", 300, 400, 40)],
        occupancy: new Map([
          [100, "c"],
          [200, "b"],
          [300, "a"],
  ...
  it("对向换位：A→B 格且 B→A 格，双双放行", () => {
    const { moves } = resolveTraffic(
      input({
        intents: [intent("a", 100, 200, 60), intent("b", 200, 100, 40)],
        occupancy: new Map([
          [100, "a"],
          [200, "b"],
        ]),
  ...
  it("头对头僵持（目标同格、互不占位）只放行一个", () => {
    ...
    const { moves } = resolveTraffic(
      input({
        intents: [intent("a", 100, 200, 60), intent("b", 300, 200, 60)],
        occupancy: new Map([
          [100, "a"],
          [300, "b"],
        ]),
...
describe("resolveTraffic — 推挤链", () => {
  ...
  it("落格排序被尊重：首选格被占（静止者）时链式推挤（深度 2）", () => {
    ...
    const { moves } = resolveTraffic(
      input({
        ...
        shoveCandidates: tile => (tile === 200 ? [250] : tile === 250 ? [260] : []),
      }),
    );
  ...
  it("推挤落格不选仲裁胜者的目标格（防同格双占）", () => {
    ...
    const { moves } = resolveTraffic(
      input({
        intents: [intent("winner", 300, 250, 100), intent("mover", 100, 200, 60)],
        occupancy: new Map([
          [300, "winner"],
          [100, "mover"],
          [200, "idler"],
...
describe("resolveTraffic — 边界", () => {
  ...
  it("同名 creep 重复意图：只取最高优一条", () => {
    const { moves } = resolveTraffic(
      input({
        intents: [intent("a", 100, 200, 40), intent("a", 100, 300, 60)],
        occupancy: new Map([[100, "a"]]),
      }),
    );
