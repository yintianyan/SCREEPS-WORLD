...
describe("buildStructurePositions — 同格多结构合并", () => {
  it("rampart 叠盾在 spawn 上：整格保持 255（任意遍历序）", () => {
    ...
    const a = toCostMap(
      buildStructurePositions([structure("spawn", 25, 24), structure("rampart", 25, 24)], [])
        .positions,
    );
    const b = toCostMap(
      buildStructurePositions([structure("rampart", 25, 24), structure("spawn", 25, 24)], [])
        .positions,
    );
  ...
  it("rampart 叠在 road 上：取通行成本更优的 road（1）", () => {
    const m = toCostMap(
      buildStructurePositions([structure("road", 10, 10), structure("rampart", 10, 10)], [])
        .positions,
    );
  ...
  it("实体 site 与障碍结构同格：障碍 255 优先于 site 的强避 50", () => {
    const m = toCostMap(
      buildStructurePositions([structure("spawn", 7, 7)], [structure("extension", 7, 7) as any])
        .positions,
    );
