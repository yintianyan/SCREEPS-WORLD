...
describe("E2E-014 旧 Schema 迁移", () => {
  ...
  it("阶段1：注入 schemaVersion=1 的旧版 Memory 后迁移执行", async () => {
    // 先跑 1 tick 让 bot 初始化
    await runner.tick();
    ...
    await runner.bot.sendConsole('Memory = {"schemaVersion":1,"creeps":{},"rooms":{"W0N1":{}}}');
  ...
  it("阶段2：运行 500 tick 后迁移完成", async () => {
    ...
    expect(memAgain.schemaVersion, "schemaVersion 应稳定不变").toBe(mem.schemaVersion);
  }, 180000);

  it("阶段3：继续 500 tick 验证稳定性", async () => {
    const snapshots = await runner.runTicks(500);
    const last = snapshots.at(-1)!;
    ...
    const migrationErrors = consoleLogs.filter(
      line => line.includes("migrate") && (line.includes("ERROR") || line.includes("FATAL")),
    );
    expect(migrationErrors, `迁移有错误：\n${migrationErrors.join("\n")}`).toHaveLength(0);

    // 精确断言：schemaVersion 稳定且等于 CONFIG.memory.schemaVersion
    const mem = await runner.bot.getMemory();
    expect(mem.schemaVersion, "schemaVersion 应保持稳定").toBe(CONFIG.memory.schemaVersion);
    expect(mem.schemaVersion, "schemaVersion 必须等于 CONFIG.memory.schemaVersion").toBe(
      CONFIG.memory.schemaVersion,
    );
