...
describe("E2E-018 Emergency Survival Mode — 进入/保持/退出", () => {
  ...
  it("bucket<100 进入、带内保持、≥500 退出；ESM 期间 P1+ 让位、bot 存活", async () => {
    ...
    expect(enterSeen, `bucket<100 应触发 ESM 进入。日志尾部: ${logs.slice(-8).join(" | ")}`).toBe(
      true,
    );
    ...
    expect(exitDuringBand, "bucket∈[100,500) 带内不应退出 ESM（保命态无滞回但退出阈为 ≥500）").toBe(
      false,
    );
