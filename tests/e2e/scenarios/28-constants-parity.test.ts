...
describe("E2E-028 常量 parity 哨兵 — support/constants ↔ 真实引擎运行时全局", () => {
  ...
  beforeAll(async () => {
    ...
    const pairs = PARITY_KEYS.map(
      k => `${k}: (typeof ${k} !== "undefined" ? ${k} : "__MISSING__")`,
    );
    await runner.bot.sendConsole(
      `console.log("CONSTPARITY " + JSON.stringify({ ${pairs.join(", ")} }));`,
    );
    const snaps = await runner.runTicks(1);
    const line = snaps.flatMap(s => s.consoleLogs).find(l => l.includes("CONSTPARITY "));
    expect(line, "未捕获 CONSTPARITY 输出 — console 注入链路失败").toBeDefined();
    runtimeDump = JSON.parse(decodeConsoleEntities(line!.split("CONSTPARITY ")[1]!));
  }, 180000);
