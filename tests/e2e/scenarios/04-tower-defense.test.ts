...
describe("E2E-004 Tower 防御", () => {
  ...
  it("RCL3 + tower 场景稳定运行 300 tick", async () => {
  ...
  it("注入 hostile creep 后 tower 发起攻击", async () => {
    ...
    if (after !== undefined && after.hits >= after.hitsMax) {
      ...
      console.log(
        `[tower-diag] threat=${JSON.stringify(
          rm.threat ?? rm.threatLevel ?? null,
        )} hostiles=${JSON.stringify(
          rm.hostiles ?? rm.lastHostileAt ?? null,
        )} towerCensus=${JSON.stringify(towers)} towerRaw=${JSON.stringify(
          ((await (runner.inspector as any).world().roomObjects("W0N1")) as any[]).filter(
            o => o.type === "tower",
          ),
        ).slice(0, 400)} colonyState=${rm.colonyState ?? "?"}`,
      );
    }
    expect(
