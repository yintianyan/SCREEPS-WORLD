...
describe("E2E-019 多房 energy 互济 — terminal 调拨", () => {
  ...
  it("Plan 活跃窗口内 self-aid 被压制（A4.4 决策权语义回归锁定）", async () => {
    ...
    for (let i = 0; i < totalStages && !aidLog; i++) {
      ...
      for (const l of logs) {
        ...
        if (l.includes("energy-aid:") || l.includes("self-aid skipped") || l.includes("terminal:"))
          console.log(`[dbg-aid] ${l}`);
      }
    ...
    expect(probeBefore, "应采集到受助房 terminal 探针").toBeDefined();
