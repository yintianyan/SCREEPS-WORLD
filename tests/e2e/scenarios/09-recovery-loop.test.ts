...
async function getCreepTtls(
  runner: ScenarioRunner,
  roomName: string,
): Promise<Array<{ name: string; role: string; ttl: number }>> {
...
describe("E2E-009 Recovery 闭环验证", () => {
  ...
  it("Phase 1: 运行到稳态（1500t）— 建立人口基线", async () => {
    ...
    console.log(
      `Phase 1 稳态基线: tick=${last.tick} creeps=${total} roles=${JSON.stringify(roles)}`,
    );
  }, 300000); // 5 分钟

  it("Phase 2: 自然 TTL 死亡波 + Recovery 恢复（2000t）— 验证自动恢复闭环", async () => {
    // 记录死亡波前的基线
  ...
  it("Phase 3: 持续稳定运行（1500t）— 验证恢复后长期稳定", async () => {
    const snapshots = await runner.runTicks(1500);
    const last = snapshots.at(-1)!;
    ...
    expect(memSize, `Phase 3 后 Memory 膨胀: ${(memSize / 1024).toFixed(1)}KB`).toBeLessThan(
      500 * 1024,
    );
