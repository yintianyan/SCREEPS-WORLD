...
function findRecoveryLogs(logs: string[]): string[] {
  return logs.filter(
    l => l.includes("recovery") || l.includes("RECOVERY") || l.includes("DEFENSE_RESPONSE"),
  );
}
...
describe("E2E-012 Military & Defense", () => {
  ...
  it("S1: NPC Invader Response — 注入 NPC invader 后威胁评估产出 + 无 JS 错误", async () => {
    // 先运行 200t 到稳态
    ...
    expect(errors, `S1 响应阶段检测到 JS 错误:\n${errors.slice(0, 3).join("\n")}`).toHaveLength(0);
  ...
  it("S2: Threat Trace Collection — 威胁存在时 decision-trace 采集 DEFENSE_PREP 记录", async () => {
    // 注入更强的威胁：2 只 [ATTACK, ATTACK, MOVE] hostile
    await injectHostile(runner, ROOM, 30, 30, ["attack", "attack", "move"], "invader-2", "invader");
    await injectHostile(runner, ROOM, 32, 32, ["attack", "move", "move"], "invader-3", "invader");
  ...
  it("S3: Defense Recovery Link — CRITICAL 威胁时 recovery-execution 触发 defense_response", async () => {
  ...
  it("S4: Remote Defense Decision — 远矿运营 + 威胁 → 决策链路无错误", async () => {
    ...
    const queueLength = Array.isArray(roomMem?.spawnQueue) ? roomMem.spawnQueue.length : 0;
    expect(queueLength, `spawnQueue 堆积 ${queueLength} 条`).toBeLessThan(20);
  ...
  it("S5: Long Stability with Defense — 3000t 连续运行无 JS 错误", async () => {
  ...
  it("S6: Memory Budget Under Threat — 威胁期间 Memory 预算安全", async () => {
    // 再注入一波威胁
    await injectHostile(runner, ROOM, 20, 20, ["ranged_attack", "move"], "raider-1", "invader");
