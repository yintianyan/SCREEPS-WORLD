...
describe("E2E-022 war 账本 — 战争全程经济不越红线（Scenario F · W3）", () => {
  ...
  it("war 授权真目标、编队孵化、全程经济不越红线", async () => {
    ...
    console.log(
      `[soak-evidence] war-ledger: firstWar=${firstWar?.tick ?? "never"} ` +
    ...
    console.log(
      `[soak-evidence] war-ledger binding: schemaVersion=43 gcl=1 collectedAt=${new Date().toISOString()}`,
    );
