...
describe("E2E-005 Global Reset 恢复韧性", () => {
  ...
  it("阶段1：500 tick 建立稳态并记录 Memory 快照", async () => {
    ...
    expect(mem.schemaVersion, "schemaVersion 应存在（Memory 迁移系统应已初始化）").toBeDefined();
  }, 180000);

  it("阶段2：继续 1000 tick 后 Memory 持久性验证", async () => {
    // 读取阶段1结束时的 Memory 基线
    const memBefore = await runner.bot.getMemory();
    const schemaBefore = memBefore.schemaVersion;
    const creepMemCountBefore = memBefore.creeps ? Object.keys(memBefore.creeps).length : 0;
    const roomMemCountBefore = memBefore.rooms ? Object.keys(memBefore.rooms).length : 0;
    ...
    expect(errorLogs, `1000 tick 内检测到 JS 错误:\n${errorLogs.join("\n")}`).toHaveLength(0);
    ...
    const creepMemCountAfter = memAfter.creeps ? Object.keys(memAfter.creeps).length : 0;
    expect(
      creepMemCountAfter,
    ...
    // room memory 数量不减
    const roomMemCountAfter = memAfter.rooms ? Object.keys(memAfter.rooms).length : 0;
    expect(
      roomMemCountAfter,
