...
describe("E2E-003 Storage 建造（RCL4）", () => {
  ...
  it("storage site 被创建（RCL4 建造链路真值，R20/T6）", async () => {
  ...
  it("Memory 持久存在且 schemaVersion 稳定", async () => {
    ...
    expect(Object.keys(mem2.rooms ?? {}).length, "Memory.rooms 应至少有当前房间").toBeGreaterThan(
      0,
    );
  }, 60000);
});
