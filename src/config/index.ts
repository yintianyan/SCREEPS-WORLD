...
export const CONFIG = {
  ...
  remote: {
    ...
    minDuration: 5000,
    /** 零交付止损的孵化投入门槛（能量）— 过承诺期仍零交付、且孵化累计投入
     * 达到此值时废弃（2026-09 实证：W36S58 冻结在 haulerNeed=1 运力仅为产出
     * 12%，18k tick 零交付、烧掉 15 万孵化，实测门因 delivered=0 豁免、空转门
     * 因编队总有成员在工作而不触发、静态门看的是开点预测 —— 三门全部漏接，
     * 只有账本里「零交付 + 高投入」这个组合能兜底）。取 10000 ≈ RCL5-6 一套
     * 完整编队（harvester+hauler+reserver）的 3 倍：给足重启/换班的机会，
     * 又不让黑洞烧过一套半编队。 */
    zeroDeliverySpawnCost: 10000,
  ...
  market: {
    ...
    /** 竞品卖盘压价步长 — 存在更便宜竞品卖单时，挂单价取竞品 ask − 此步长
     * （市场无自动撮合，买家只吃最低 ask；不压价则永远排不上队）。 */
    sellUndercutStep: 0.01,
    /** 单笔挂单量上限（手续费与流动性暴露封顶）。 */
    maxOrderAmount: 5000,
