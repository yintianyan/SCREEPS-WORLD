/**
 * Tower 目标选择 — 纯函数（P1-3）。
 * 背景：旧实现 findClosestByRange 只打最近目标，不识别奶妈、不集火、不考虑塔伤
 * 随距离衰减，易被带 HEAL 单位奶穿空耗能量。选择原则（全塔集火同一目标）：
 * ① 奶妈优先（先点掉治疗源，避免拉锯耗空）；② 有效血量（hits + 自愈缓冲估算）
 * 最低优先（优先击杀最脆单位换取减员）；③ 距塔近优先（塔伤随距离衰减，近处
 * 收益最大）。纯函数无副作用；系统层收集摘要、异常时回退 findClosestByRange。
 */
