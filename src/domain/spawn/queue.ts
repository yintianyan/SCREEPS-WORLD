...
/**
 * 撤销房间内某角色的全部待处理请求（返回移除数）。
 * 常规出队路径只有孵化成功/TTL 过期/重试隔离；需求前提消失（威胁清除后的 defender、
 * 状态翻转后的 upgrader）时残留请求会在 TTL 窗口（最长 1000 tick）内继续被孵 —
 * 幽灵需求浪费能量，调用方在每 tick 需求评估前主动撤销。
 */
 ...
 * 互补：前者按 home+role 精细过滤，本函数按 home 整体清空。
 * 唯一调用方 expansion-manager；禁止任何模块直接对 spawnQueue splice
 * （队列属主是 spawn-manager，见 spawnQueue-splice 守卫测试）。
 */
...
/** 排序：priority 升序（P0 在前）→ 有 replaceBy 的替换请求优先 → createdAt 升序。 */
...
/** 统计房间内某角色的待处理请求数（不含孵化中）。可选 home 过滤：sponsor 房代孵
 * 他房 creep（扩张拓荒）时请求寄宿 sponsor 队列但 home 指向目标房 — 不过滤会
 * 污染 sponsor 自身的人口预算。 */
