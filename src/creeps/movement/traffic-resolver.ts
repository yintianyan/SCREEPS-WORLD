 ...
 * 锚定声明与 creep 占位表；输出：最终应签发的移动指令（含推挤指令）。
 * 解算规则按序：1. 同格仲裁（最高 priority 胜，平局按登记序）；2. 跟车放行（目标格占用者
 * 本轮已被批准移走）；3. 对向换位（A→B 且 B→A 双双放行，引擎同 tick 互穿，不支持则原地
 * 一 tick 后走推挤路径自动降级）；4. 推挤链（无意图静止者被严格更高优者推挤，链深上限 2）；
 * 5. 疲劳/锚定豁免（fatigue>0 或锚定优先级 ≥ 移动方者视为本 tick 硬墙）。
 * 坐标统一 packed 格式（x*50+y），与 movement 层其余模块一致。
 */
