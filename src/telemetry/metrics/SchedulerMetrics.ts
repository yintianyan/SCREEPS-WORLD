...
export function collectSchedulerMetrics(
  ...
  try {
    ...
    if (executedCount > 0)
      incrementCounter("screeps_scheduler_executed_processes_total", executedCount);
    if (skippedCount > 0)
      incrementCounter("screeps_scheduler_skipped_processes_total", skippedCount);
    if (deferredCount > 0)
      incrementCounter("screeps_scheduler_deferred_processes_total", deferredCount);
