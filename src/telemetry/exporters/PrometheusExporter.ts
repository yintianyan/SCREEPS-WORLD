  ...
  if (metric.kind === "counter") {
    ...
    for (const histEntry of metric.histogramEntries ?? []) {
      ...
      for (const bc of histEntry.bucketCounts) {
        lines.push(
          `${metric.name}_bucket{le="${bc.threshold}"${labelStrBase ? `,${labelStrBase.slice(1, -1)}` : ""}} ${bc.count}`,
        );
      }
      lines.push(
        `${metric.name}_bucket{le="+Inf"${labelStrBase ? `,${labelStrBase.slice(1, -1)}` : ""}} ${histEntry.count}`,
      );
